...
const SignupQuiz: React.FC<SignupQuizProps> = ({ userName, userId, onComplete, onCancel }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-white/90 backdrop-blur-sm rounded-3xl p-8 shadow-2xl max-w-md w-full mx-auto relative"
    >
      {/* キャンセルボタン */}
      <button
        onClick={onCancel}
      ...
      <div className="flex gap-3">
        {currentQuestionIndex > 0 && (
          <motion.button
            ...
            className="flex-1 bg-gray-100 text-gray-600 font-semibold py-3 px-6 rounded-xl hover:bg-gray-200 transition-all duration-300 flex items-center justify-center gap-2"
          >
        ...
        <button
          onClick={onCancel}
          className="flex-1 bg-red-100 text-red-600 font-semibold py-3 px-6 rounded-xl hover:bg-red-200 transition-all duration-300"
        >
          キャンセル
        </button>
      </div>
